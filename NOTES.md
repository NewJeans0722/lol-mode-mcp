...
- **2026-07-22**(例行更新檢查:官方無新版,修掉自己上一版留下的回歸):
  使用者要求「看看官方有沒有更新」,照 `UPDATE_SOP.md` 跑
  `scripts/check_update.py`。結論:**官方沒出新 patch**
  (ddragon 16.14.1 / cdragon 16.14 / wiki V26.14,與 07-20 那次相同),
  但自檢跳出 1 行翻不出來 —— 梅爾被動的
  「Overwhelm additional stack base damage」。
  - **成因是我們自己**:07-20 的 `9ee8337` 把 `{{pp|4;5;6;7}}` 的分級值
    從 `4;5;6;7` 改排成 `4/5/6/7`,而 `mapchanges_zh.json` 的 key 是
    **清理後的英文原句**,舊 key 還帶著分號 → 對不上 → 退回 🔤 英文。
    教訓:**動 `wikitext.py` 的輸出格式 = 動到對照檔的 key**,
    改完要立刻跑一次 check_update,不要等下一版才發現。
  - 修法:key 的 `4;5;6;7` → `4/5/6/7`。順手把譯文裡的 "Overwhelm"
    換成官方台服名「奪目」(ddragon zh_TW Mel 被動原文確認),句子也
    改精確:「奪目每額外一層的基礎傷害改為 4/5/6/7(隨無垠天照的等級變化)」
    (`Golden Eclipse` 留英文,執行期由 name_map 換成台服技能名)。
  - 掃過整個對照檔:含分號數列的 key **只有這一筆**;另有 7 筆 key 對不上
    現行來源,是**舊版數值的歷史殘留**(如觸手傷害 4−20% 已被 10−30%
    取代),留著不影響比對,先不刪。
  - 其他 SOP 項目全綠:強化說明無 `@` 漏出(281)、Mayhem 220 筆 100%
    中文、mayhem_zh key 全對得上;鍛體流 20%–100% 與 wiki 一致;
    官方繁中 patch notes 的英雄名仍各自獨立(HTML 結構沒再變)。
  - 驗收:139 tests 全過;`arena_balance('梅爾')` 與 `patch_notes()`
    實跑輸出確認。
